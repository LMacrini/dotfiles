...
# Read configuration from info.json
config=$(cat "$info")
...
# Set up authentication header if GITHUB_TOKEN is available
auth_header=()
...
update_github() {
  local repo query version oldVersion releases
  ...
  if [[ -z "$repo" ]]; then
    echo "❌ Error: 'repo' must be set for GitHub updates"
    exit 1
  fi

  if [[ -z "$query" || "$query" == "null" ]]; then
    echo "❌ Error: 'query' must be set for GitHub updates"
    exit 1
  fi
  ...
  if [[ -z "$version" || "$version" == "null" ]]; then
    echo "❌ No version found using query: $query"
    exit 1
  fi
  ...
  # Check if this is a multi-platform package
  if jq -e '.platforms' <<< "$config" > /dev/null; then
    update_github_multiplatform "$repo" "$version"
  else
    update_github_single "$repo" "$version"
  fi
}
...
update_github_single() {
  ...
  if [[ -z "$url_template" ]]; then
    echo "❌ Error: 'url_template' must be set for single-file GitHub updates"
    exit 1
  fi
  ...
  # Handle version_clean (removes 'v' prefix)
  version_clean="${version#v}"
  url="${url//\{version_clean\}/$version_clean}"
...
update_github_multiplatform() {
  ...
  jq -c '.platforms | keys[]' <<< "$config" | while read -r platform; do
    ...
    if [[ -z "$file" || "$file" == "null" ]]; then
      echo "❌ Error: 'file' must be set for platform '$platform'"
      exit 1
    fi
...
update_api() {
  ...
  if [[ -z "$api_url" ]]; then
    echo "❌ Error: 'api_url' must be set for API updates"
    exit 1
  fi

  response=$(curl -fsSL "$api_url")

  # Extract version and URL using configurable JSON paths
  version_path=$(jq -r '.version_path // ".version"' <<< "$config")
  url_path=$(jq -r '.url_path // ".url"' <<< "$config")

  newVersion=$(jq -r "$version_path" <<< "$response")
  newUrl=$(jq -r "$url_path" <<< "$response")

  if [[ -z "$newVersion" || -z "$newUrl" || "$newVersion" == "null" || "$newUrl" == "null" ]]; then
    echo "❌ Failed to fetch new version or URL from API"
    exit 1
  fi
  ...
  if nix store prefetch-file "$newUrl" --json > "$tmp"; then
    ...
    echo "❌ Failed to prefetch $newUrl"
    rm -f "$tmp"
    exit 1
  fi

  # Update info.json with new version, URL, and hash
  jq \
...
# Main execution based on update type
case "$update_type" in
  "github")
    update_github
    ;;
